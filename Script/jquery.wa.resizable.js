	...
	_bindEvent: function (target, orient) {
	    ...
	    target.bind('mousedown.' + me.name, function (e) {
	        ...
	        if (options.simulate) {
	            ...
	            }).appendTo('body').offset({ left: elementOffset.left - simulateBorderWidth, top: elementOffset.top - simulateBorderWidth });
	        } else {
	        ...
	        $(document).bind('mousemove.' + me.name + me.guid, function (e) {
	            ...
	            if (orient.indexOf('s') > -1) {
	                ...
	                top = Math.min(top, containerOffset.top + containerHeight - elementOuterHeight - simulateBorderWidth);
	                resizeTarget.height((resizeTarget == me.element ? elementHeight : elementOuterHeight) + top - elementOffset.top);
	            }
	            if (orient.indexOf('n') > -1) {
	                top = Math.min(top, elementOffset.top + elementOuterHeight - options.minHeight);
	                top = Math.max(top, containerOffset.top + simulateBorderWidth);
	                resizeTarget.offset({ top: top - (resizeTarget == me.element ? 0 : simulateBorderWidth) });
	                resizeTarget.height((resizeTarget == me.element ? elementHeight : elementOuterHeight) + elementOffset.top - top);
	            }
	            if (orient.indexOf('w') > -1) {
	                left = Math.min(left, elementOffset.left + elementOuterWidth - options.minWidth);
	                left = Math.max(left, containerOffset.left + simulateBorderWidth);
	                resizeTarget.offset({ left: left - (resizeTarget == me.element ? 0 : simulateBorderWidth) });
	                resizeTarget.width((resizeTarget == me.element ? elementWidth : elementOuterWidth) + elementOffset.left - left);
	            }
	            if (orient.indexOf('e') > -1) {
	                left = Math.max(left, elementOffset.left - elementOuterWidth + options.minWidth);
	                left = Math.min(left, containerOffset.left + containerWidth - elementOuterWidth - simulateBorderWidth);
	                resizeTarget.width((resizeTarget == me.element ? elementWidth : elementOuterWidth) + left - elementOffset.left);
	            }
